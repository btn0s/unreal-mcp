...
def register_blueprint_node_tools(mcp: FastMCP):
    ...
    def add_blueprint_event_node(
        ...
        Args:
            ...
            event_name: Name of the event. Use 'Receive' prefix for standard events:
                       ...
                       - etc.
            node_position_json: JSON-encoded [X, Y] position in the graph (default: "[0, 0]")
            
        Returns:
            Dict with status="success" and result containing the node ID and success status
        """
        from unreal_mcp_server import get_unreal_connection
        
        try:
            # Parse node_position JSON
            try:
                node_position = json.loads(node_position_json)
                if not isinstance(node_position, list) or len(node_position) != 2:
                    return _canonical_response(None, "node_position_json must be a JSON array of 2 numbers [X, Y]")
            except json.JSONDecodeError:
                return _canonical_response(None, f"Invalid node_position_json: {node_position_json}")
    ...
    def add_blueprint_function_node(
        ...
        Args:
            ...
            params_json: JSON-encoded dict of optional parameters to set on the function node (default: "{}")
            node_position_json: JSON-encoded [X, Y] position in the graph (default: "[0, 0]")
        ...
        try:
            ...
            try:
                ...
                if not isinstance(params, dict):
                    return _canonical_response(None, "params_json must be a JSON object")
            except json.JSONDecodeError:
                return _canonical_response(None, f"Invalid params_json: {params_json}")
    ...
    def add_blueprint_get_self_component_reference(
        ...
        try:
            ...
            logger.error(f"Error adding self component reference node: {e}")
            return _canonical_response(None, str(e))
    ...
    def add_blueprint_self_reference(
        ...
        try:
            ...
            logger.error(f"Error adding self reference node: {e}")
            return _canonical_response(None, str(e))
